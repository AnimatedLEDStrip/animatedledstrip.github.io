---
title: Animations
nav_order: 2
has_children: true
---

# Animations

- [AlterFade](animations/AlterFade)
- [Alternate](animations/Alternate)
- [Bounce](animations/Bounce)
- [Bounce to Color](animations/Bounce-to-Color)
- [Bubble Sort](animations/Bubble-Sort)
- [Cat Toy](animations/Cat-Toy)
- [Cat Toy to Color](animations/Cat-Toy-to-Color)
- [Color](animations/Color)
- [Fade to Color](animations/Fade-to-Color)
- [Fireworks](animations/Fireworks)
- [Heap Sort](animations/Heap-Sort)
- [Insertion Sort](animations/Insertion-Sort)
- [Merge Sort (Parallel)](animations/Merge-Sort-Parallel)
- [Merge Sort (Sequential)](animations/Merge-Sort-Sequential)
- [Meteor](animations/Meteor)
- [Pixel Marathon](animations/Pixel-Marathon)
- [Pixel Run](animations/Pixel-Run)
- [Plane Run](animations/Plane-Run)
- [Quick Sort (Parallel)](animations/Quick-Sort-Parallel)
- [Quick Sort (Sequential)](animations/Quick-Sort-Sequential)
- [Ripple](animations/Ripple)
- [Runway Lights](animations/Runway-Lights)
- [Runway Lights to Color](animations/Runway-Lights-to-Color)
- [Shell Sort](animations/Shell-Sort)
- [Smooth Chase](animations/Smooth-Chase)
- [Smooth Fade](animations/Smooth-Fade)
- [Sparkle](animations/Sparkle)
- [Sparkle Fade](animations/Sparkle-Fade)
- [Sparkle to Color](animations/Sparkle-to-Color)
- [Splat](animations/Splat)
- [Stack](animations/Stack)
- [Stack Overflow](animations/Stack-Overflow)
- [Wave](animations/Wave)
- [Wipe](animations/Wipe)

# Groups

- [Sorting](animations/Sorting)
